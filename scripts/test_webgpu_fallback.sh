...
It launches a dedicated steklo-gui process with:
  - front_end = 'WebGpu'
...
BINARY="/Applications/Steklo.app/Contents/MacOS/steklo-gui"
BASE_CONFIG="/Applications/Steklo.app/Contents/Resources/steklo.lua"
...
tmp_dir="$(mktemp -d -t steklo-webgpu-fallback.XXXXXX)"
cfg_file="$tmp_dir/steklo-fallback.lua"
log_file="$tmp_dir/steklo-fallback.log"
vmmap_file="$tmp_dir/vmmap.txt"
summary_file="$tmp_dir/vmmap-summary.txt"
class_name="fun.tw93.steklo.fallback.$RANDOM.$RANDOM"
...
before_pids="$(pgrep -x steklo-gui || true)"
if [[ -n "$before_pids" ]]; then
  if [[ "$FORCE_KILL_EXISTING" -eq 1 ]]; then
    pkill -x steklo-gui >/dev/null 2>&1 || true
    sleep 1
    before_pids=""
  else
    echo "Found existing steklo-gui process(es): $before_pids" >&2
    echo "Close Steklo first, or rerun with --force-kill-existing." >&2
    exit 1
  fi
fi
...
for _ in $(seq 1 40); do
  sleep 0.25
  now_pids="$(pgrep -x steklo-gui || true)"
...
if [[ -z "$pid" ]]; then
  echo "FAIL: Could not find a newly launched steklo-gui process." >&2
