...
# Notarization script for Steklo macOS app
...
#    - STEKLO_NOTARIZE_APPLE_ID: Your Apple ID email
#    - STEKLO_NOTARIZE_TEAM_ID: Your Team ID (10 characters)
#    - STEKLO_NOTARIZE_PASSWORD: App-specific password (not your Apple ID password)
#
...
if ! codesign -v "$APP_BUNDLE" 2>/dev/null; then
	echo "Error: App is not signed. Run build with STEKLO_SIGNING_IDENTITY set."
	exit 1
fi
...
APPLE_ID="${STEKLO_NOTARIZE_APPLE_ID:-}"
TEAM_ID="${STEKLO_NOTARIZE_TEAM_ID:-}"
PASSWORD="${STEKLO_NOTARIZE_PASSWORD:-}"

# If not set via env, try to read from Keychain
if [[ -z "$APPLE_ID" ]]; then
	echo "Checking Keychain for notarization credentials..."
	APPLE_ID=$(security find-generic-password -s "steklo-notarize-apple-id" -w 2>/dev/null || true)
fi

if [[ -z "$PASSWORD" ]]; then
	PASSWORD=$(security find-generic-password -s "steklo-notarize-password" -w 2>/dev/null || true)
fi
...
if [[ -z "$APPLE_ID" || -z "$PASSWORD" || -z "$TEAM_ID" ]]; then
	...
	echo "  export STEKLO_NOTARIZE_APPLE_ID='your-apple-id@example.com'"
	echo "  export STEKLO_NOTARIZE_TEAM_ID='YOURTEAMID'"
	echo "  export STEKLO_NOTARIZE_PASSWORD='xxxx-xxxx-xxxx-xxxx'"
	echo ""
	echo "Or store in Keychain:"
	echo "  security add-generic-password -s 'steklo-notarize-apple-id' -a 'steklo' -w 'your-apple-id@example.com'"
	echo "  security add-generic-password -s 'steklo-notarize-password' -a 'steklo' -w 'your-app-specific-password'"
	echo ""
