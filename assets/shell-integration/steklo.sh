...
# Remote machines typically don't have the "steklo" terminfo entry, so when
# SSHing from a Steklo local shell (TERM=steklo), zsh plugins on the remote side
...
# Override TERM for ssh sessions the same way Steklo's built-in SSH domain does.
if [[ "${TERM:-}" == "steklo" ]]; then
