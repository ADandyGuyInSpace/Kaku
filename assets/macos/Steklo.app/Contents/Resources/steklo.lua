...
if os.getenv('STEKLO_STRICT_CONFIG') == '1' and wezterm.config_builder then
...
local steklo_state_dir = home_dir and (home_dir .. "/.config/steklo") or nil
local lazygit_state_file = steklo_state_dir and (steklo_state_dir .. "/lazygit_state.json") or nil
...
local ai_fix_toml_path = steklo_state_dir and (steklo_state_dir .. "/assistant.toml") or nil
...
local ai_fix_jobs_dir = steklo_state_dir and (steklo_state_dir .. "/ai_jobs") or "/tmp"
...
local function ai_debug_log(message)
  ...
  local file = io.open("/tmp/steklo_ai_debug.log", "a")
  if not file then
    return
  end
...
local function inject_ai_notice(pane, headline, detail, suggested_command)
  ...
  local summary_line = "\27[38;5;141m╭─ Steklo Assistant\27[0m  \27[1m" .. summary .. "\27[0m"
  local command_line = ""
  if cmd ~= "" then
...
local function inject_ai_status(pane, message)
  ...
  local line = "\27[38;5;141m╰─ Steklo Assistant\27[0m  \27[38;5;244m" .. summary .. "\27[0m"
  local output = "\r\n" .. line .. "\r\n"
  pcall(function()
    pane:inject_output(output)
  end)
...
local function show_ai_loading_toast(window, pane)
  ...
  pcall(function()
    window:perform_action(wezterm.action.EmitEvent("steklo-toast-ai-analyzing"), pane)
  end)
end
...
local function ensure_steklo_state_dir()
  if not steklo_state_dir or steklo_state_dir == "" then
    return
  end
  os.execute(string.format("mkdir -p %q", steklo_state_dir))
end
...
local function maybe_show_lazygit_hint(window, pane)
  ...
  show_lazygit_toast(window, pane, "steklo-toast-try-lazygit")
...
local function launch_lazygit(window, pane)
  pane = resolve_active_pane(window, pane)
  if not pane then
    show_lazygit_toast(window, pane, "steklo-toast-lazygit-no-pane")
    return
  end

  local path = pane_cwd(pane)
  if path == "" then
    show_lazygit_toast(window, pane, "steklo-toast-lazygit-no-cwd")
    return
  end

  local repo_root = detect_git_repo_root(path)
  if not repo_root then
    show_lazygit_toast(window, pane, "steklo-toast-lazygit-not-git")
    return
  end

  local lazygit_cmd = resolve_lazygit_command()
  if not lazygit_cmd then
    show_lazygit_toast(window, pane, "steklo-toast-lazygit-missing")
    return
  end
  ...
  if not ok then
    show_lazygit_toast(window, pane, "steklo-toast-lazygit-dispatch-failed")
    return
  end
  mark_repo_lazygit_used(repo_root)
end
...
local function launch_yazi(window, pane)
  pane = resolve_active_pane(window, pane)
  if not pane then
    show_yazi_toast(window, pane, "steklo-toast-yazi-no-pane")
    return
  end

  local yazi_cmd = resolve_yazi_command()
  if not yazi_cmd then
    show_yazi_toast(window, pane, "steklo-toast-yazi-missing")
    return
  end
  ...
  if not ok then
    show_yazi_toast(window, pane, "steklo-toast-yazi-dispatch-failed")
  end
end
...
wezterm.on('steklo-launch-lazygit', function(window, pane)
  launch_lazygit(window, pane)
end)

wezterm.on('steklo-launch-yazi', function(window, pane)
  launch_yazi(window, pane)
end)

wezterm.on('steklo-ai-apply-last-fix', function(window, pane)
  pane = resolve_active_pane(window, pane)
  if not pane then
    return
  end
...
wezterm.on('user-var-changed', function(window, pane, name, value)
  ...
  if name ~= "steklo_last_exit_code" then
    return
  end
  if not pane then
  ...
  local failed_command = trim_surrounding_whitespace(vars.steklo_last_cmd or "")
  if failed_command == "" then
    ai_debug_log("user-var-changed ignored missing steklo_last_cmd")
    return
  end
...
config.color_schemes['Steklo Theme'] = steklo_theme
if not config.color_scheme then
  config.color_scheme = 'Steklo Theme'
end
...
config.keys = {
  ...
  -- Cmd+Shift+A: open Steklo AI settings in current pane
  {
    key = 'A',
    mods = 'CMD|SHIFT',
    action = wezterm.action.EmitEvent('run-steklo-ai-config'),
  },

  -- Cmd+Shift+E: apply latest Steklo Assistant suggestion for the active pane
  {
    key = 'E',
    mods = 'CMD|SHIFT',
    action = wezterm.action.EmitEvent('steklo-ai-apply-last-fix'),
  },
  ...
  -- Cmd+Shift+G: launch lazygit in current pane
  {
    key = 'G',
    mods = 'CMD|SHIFT',
    action = wezterm.action.EmitEvent('steklo-launch-lazygit'),
  },
  ...
  -- Cmd+Shift+Y: launch yazi in current pane
  {
    key = 'Y',
    mods = 'CMD|SHIFT',
    action = wezterm.action.EmitEvent('steklo-launch-yazi'),
  },
...
wezterm.on('gui-startup', function(cmd)
  ...
  local state_file = home .. "/.config/steklo/state.json"
  local is_first_run = false
  local needs_update = false

  local function ensure_state_dir()
    os.execute("mkdir -p " .. home .. "/.config/steklo")
  end
