...
impl Tool {
    fn label(&self) -> &'static str {
        match self {
            Tool::StekloAssistant => "Steklo Assistant",
    ...
    fn config_path(&self) -> PathBuf {
        ...
        match self {
            Tool::StekloAssistant => assistant_config::assistant_toml_path().unwrap_or_else(|_| {
                config::HOME_DIR
                    .join(".config")
                    .join("steklo")
                    .join("assistant.toml")
...
impl ToolState {
    fn load(tool: Tool) -> Self {
        let path = if tool == Tool::StekloAssistant {
        ...
        if tool != Tool::StekloAssistant && !path.exists() {
            return ToolState {
                tool,
                installed: false,
                fields: Vec::new(),
        ...
        let fields = match tool {
            Tool::StekloAssistant => extract_steklo_assistant_fields(&raw),
            Tool::ClaudeCode => {
...
/// Configuration for the Steklo built-in AI assistant.
///
/// This struct holds the configuration for Steklo's AI-powered command analysis
...
fn parse_steklo_assistant_config(raw: &str) -> StekloAssistantConfig {
    ...
    StekloAssistantConfig::new(enabled, api_key, model, base_url)
}

fn get_steklo_assistant_api_key() -> Option<String> {
    ...
    let raw = std::fs::read_to_string(&path)
        ...
        .ok()?;
    let cfg = parse_steklo_assistant_config(&raw);
...
fn extract_steklo_assistant_fields(raw: &str) -> Vec<FieldEntry> {
    let cfg = parse_steklo_assistant_config(raw);
    vec![
        FieldEntry {
            key: "Enabled".into(),
...
fn write_steklo_assistant_config(path: &Path, cfg: &StekloAssistantConfig) -> anyhow::Result<()> {
    let mut out = String::new();
    out.push_str("# Steklo Assistant configuration\n");
    out.push_str(
...
fn save_steklo_assistant_field(field_key: &str, new_val: &str) -> anyhow::Result<()> {
    let path = assistant_config::ensure_assistant_toml_exists()?;
    ...
    let updated = match field_key {
        "Enabled" => {
            ...
            StekloAssistantConfig::new(enabled, cfg.api_key(), cfg.model(), cfg.base_url())
        }
        "Model" => {
            ...
            StekloAssistantConfig::new(cfg.is_enabled(), cfg.api_key(), model, cfg.base_url())
        }
        "Base URL" => {
            ...
            StekloAssistantConfig::new(cfg.is_enabled(), cfg.api_key(), cfg.model(), base_url)
        }
        "API Key" => StekloAssistantConfig::new(
            cfg.is_enabled(),
            new_val.trim(),
            cfg.model(),
            cfg.base_url(),
    ...
    write_steklo_assistant_config(&path, &updated)
}
...
/// Fetch models.dev data, cached to ~/.cache/steklo/models.json.
/// No TTL — use `r` key in TUI to force refresh.
fn load_models_dev_json() -> Option<serde_json::Value> {
    let cache_dir = config::HOME_DIR.join(".cache").join("steklo");
...
impl App {
    ...
    fn start_edit(&mut self) {
        ...
        self.edit_buf = if field.value == "—" {
            // Empty placeholder
            String::new()
        } else if tool.tool == Tool::StekloAssistant && field.key == "API Key" {
            get_steklo_assistant_api_key().unwrap_or_else(String::new)
    ...
    /// Open a shell command in a new Steklo tab (preferred) or fall back to Terminal.app.
    fn open_in_terminal(&mut self, cmd: &str) {
        // Prefer a new tab in the current Steklo window.
        // steklo cli spawn reads WEZTERM_PANE from the environment to target the right window.
        // Append `exec $SHELL` so the pane stays alive after the command finishes.
        let shell = std::env::var("SHELL").unwrap_or_else(|_| "/bin/zsh".to_string());
        let shell_cmd = format!("{}; exec \"{}\"", cmd, shell);
        let steklo_status = std::process::Command::new("steklo")
            .args(["cli", "spawn", "--", &shell, "-c", &shell_cmd])
            .stdin(std::process::Stdio::null())
            .stdout(std::process::Stdio::null())
            .stderr(std::process::Stdio::null())
            .status();
        if steklo_status.as_ref().is_ok_and(|status| status.success()) {
            self.status_msg = Some("Opening in new Steklo tab...".into());
            return;
        }
...
fn save_field(tool: Tool, field_key: &str, new_val: &str) -> anyhow::Result<()> {
    if tool == Tool::StekloAssistant {
        return save_steklo_assistant_field(field_key, new_val);
    }
    ...
    match tool {
        Tool::StekloAssistant => unreachable!("Steklo Assistant is handled before JSON parsing"),
        Tool::Gemini => {
            if field_key == "Model" {
    ...
    if is_jsonc_path(&path) {
        log::info!(
            "Note: {} comments will be removed when Steklo rewrites this file.",
            path.display()
        );
    }
