...
mod imp {
    ...
    pub fn run(_yes: bool) -> anyhow::Result<()> {
        bail!("`steklo reset` is currently supported on macOS only")
    }
}
...
mod imp {
    ...
    const STEKLO_SOURCE_PATTERN: &str = "steklo/zsh/steklo.zsh";
    const STEKLO_LEGACY_INLINE_MARKER: &str = "# Steklo Shell Integration";
    const STEKLO_LEGACY_INLINE_VAR: &str = "STEKLO_ZSH_DIR";
    const STEKLO_LEGACY_SYNTAX_HINT: &str = "zsh-syntax-highlighting/zsh-syntax-highlighting.zsh";

    const STEKLO_GIT_DEFAULTS: &[(&str, &str)] = &[
        ("core.pager", "delta"),
    ...
    pub fn run(yes: bool) -> anyhow::Result<()> {
        ...
        remove_dir_if_exists(
            ...
            "removed Steklo backup directory",
            &mut report,
        )?;
        remove_empty_steklo_config_dir(&mut report)?;

        report.print();

        println!("\n⚠️  Shell restart required.");
        println!("ℹ️  Tools preserved in ~/.config/steklo/zsh/\n");
        ...
        if !yes && io::stdin().is_terminal() {
            ...
            if answer.is_empty() || answer == "y" || answer == "yes" {
                println!("\nRestarting shell... 👋");
                println!("Tip: Run 'steklo init' to restore integration");
                let shell = std::env::var("SHELL").unwrap_or_else(|_| "/bin/zsh".to_string());
                let err = std::process::Command::new(&shell).arg("-l").exec();
                bail!("failed to restart shell: {}", err);
            } else {
                println!("\nRun 'exec zsh' when ready. Restore with 'steklo init'");
            }
        } else {
            println!("Run 'exec zsh' to restart. Restore with 'steklo init'");
        }
    ...
    fn confirm_reset(yes: bool) -> anyhow::Result<()> {
        ...
        println!(
            "This will remove Steklo shell integration and reset Steklo-managed git defaults."
        );
    ...
    fn config_home() -> PathBuf {
        home_dir().join(".config").join("steklo")
    }
    ...
    fn remove_zsh_integration(report: &mut ResetReport) -> anyhow::Result<()> {
        ...
        let filtered: Vec<&str> = original
            .lines()
            .filter(|line| !line.contains(STEKLO_SOURCE_PATTERN))
            .collect();
        ...
        if !removed_source_line && !removed_legacy_block {
            report.skipped(format!(
                "no Steklo shell integration found in {}",
                zshrc.display()
            ));
            return Ok(());
        }
        ...
        if removed_source_line && removed_legacy_block {
            report.changed(format!(
                "removed Steklo source line and legacy inline block from {}",
                zshrc.display()
            ));
        } else if removed_source_line {
            report.changed(format!(
                "removed Steklo source line from {}",
                zshrc.display()
            ));
        } else {
            report.changed(format!(
                "removed legacy inline Steklo block from {}",
                zshrc.display()
            ));
        }
        Ok(())
    ...
    fn remove_steklo_shell_dir(report: &mut ResetReport) -> anyhow::Result<()> {
        let steklo_init = config_home().join("zsh").join("steklo.zsh");
        if steklo_init.exists() {
            std::fs::remove_file(&steklo_init)
                .with_context(|| format!("remove {}", steklo_init.display()))?;
            report.changed(format!("removed {}", steklo_init.display()));
        } else {
            report.skipped(format!("{} not found", steklo_init.display()));
        }
        Ok(())
    }
    ...
    fn cleanup_git_delta_defaults(report: &mut ResetReport) -> anyhow::Result<()> {
        ...
        for (key, expected) in STEKLO_GIT_DEFAULTS {
        ...
        if removed.is_empty() {
            report.skipped("no Steklo-managed git defaults to remove");
        } else {
    ...
    fn cleanup_theme_block(report: &mut ResetReport) -> anyhow::Result<()> {
        let config_path = config_home().join("steklo.lua");
        ...
        let (after_managed, changed_managed) =
            strip_theme_block(&original, "-- ===== Steklo Theme Defaults (managed) =====");
        let (after_legacy, changed_legacy) =
            strip_theme_block(&after_managed, "-- ===== Steklo Theme =====");

        if !changed_managed && !changed_legacy {
            report.skipped("no managed Steklo theme block found in ~/.config/steklo/steklo.lua");
            return Ok(());
        }

        std::fs::write(&config_path, after_legacy)
            .with_context(|| format!("write {}", config_path.display()))?;
        report.changed("removed managed Steklo theme block from ~/.config/steklo/steklo.lua");
        Ok(())
    }
    ...
    /// Strips the old inline Steklo shell integration block that was written directly
    /// into .zshrc in early Steklo versions (before the source-based steklo.zsh approach).
    /// The block starts with `# Steklo Shell Integration`, contains `STEKLO_ZSH_DIR`, and
    ...
    fn strip_legacy_inline_block(content: &str) -> (String, bool) {
        ...
        while i < lines.len() {
            ...
            if current == STEKLO_LEGACY_INLINE_MARKER {
                let mut j = i + 1;
                let mut saw_steklo_var = false;
                let mut saw_syntax_line = false;
                let mut end = None;

                while j < lines.len() {
                    let line = lines[j];
                    if line.contains(STEKLO_LEGACY_INLINE_VAR) {
                        saw_steklo_var = true;
                    }
                    if line.contains(STEKLO_LEGACY_SYNTAX_HINT) {
    ...
    fn remove_empty_steklo_config_dir(report: &mut ResetReport) -> anyhow::Result<()> {
        let dir = config_home();
        if !dir.exists() {
            return Ok(());
        }
