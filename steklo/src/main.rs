...
#[derive(Debug, Parser)]
#[command(
    about = "Steklo Terminal Emulator\nhttp://github.com/tw93/Steklo",
    version = wezterm_version()
)]
pub struct Opt {
    /// Skip loading steklo.lua
    #[arg(long, short = 'n')]
    skip_config: bool,
...
enum SubCommand {
    ...
    #[command(
        name = "ai",
        about = "Manage Steklo Assistant and AI coding tools configuration"
    )]
    Ai(ai_config::AiConfigCommand),

    #[command(name = "config", about = "Open and edit user steklo.lua configuration")]
    Config(config_cmd::ConfigCommand),

    #[command(name = "init", about = "Initialize Steklo shell integration")]
    Init(init::InitCommand),

    #[command(
        name = "update",
        about = "Download and install the latest Steklo release automatically"
    )]
    Update(update::UpdateCommand),

    #[command(
        name = "reset",
        about = "Reset Steklo shell integration and managed defaults"
    )]
...
fn select_main_menu_command() -> anyhow::Result<Option<SubCommand>> {
    ...
    const MENU_ITEMS: [(&str, &str, MenuChoice); 5] = [
        (
            "ai",
            "Manage Steklo Assistant and AI tool settings",
            MenuChoice::Ai,
        ),
        (
            "config",
            "Open ~/.config/steklo/steklo.lua",
            MenuChoice::Config,
        ),
        ...
        (
            "reset",
            "Remove Steklo shell integration and managed defaults",
            MenuChoice::Reset,
        ),
    ];
...
fn resolve_gui_executable(exe_name: &str) -> anyhow::Result<PathBuf> {
    ...
    #[cfg(target_os = "macos")]
    {
        candidates.push(PathBuf::from("/Applications/Steklo.app/Contents/MacOS").join(exe_name));
        candidates.push(
            config::HOME_DIR
                .join("Applications")
                .join("Steklo.app")
