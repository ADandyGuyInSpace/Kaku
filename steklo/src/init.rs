...
mod imp {
    ...
    pub fn run(_update_only: bool) -> anyhow::Result<()> {
        bail!("`steklo init` is currently supported on macOS only")
    }
}
...
mod imp {
    ...
    pub fn run(update_only: bool) -> anyhow::Result<()> {
        ...
        install_steklo_wrapper().context("install steklo wrapper")?;

        let script = resolve_setup_script()
            .ok_or_else(|| anyhow!("failed to locate setup_zsh.sh for Steklo initialization"))?;

        let mut cmd = Command::new("/bin/bash");
        cmd.arg(&script).env("STEKLO_INIT_INTERNAL", "1");
        if update_only {
            cmd.arg("--update-only");
        }
        let status = cmd
        ...
        bail!("steklo init failed with status {}", status);
    }

    fn install_steklo_wrapper() -> anyhow::Result<()> {
        ...
        let preferred_bin = resolve_preferred_steklo_bin()
            .unwrap_or_else(|| PathBuf::from("/Applications/Steklo.app/Contents/MacOS/steklo"));
...
"#
    ...
    fn resolve_preferred_steklo_bin() -> Option<PathBuf> {
        ...
        if let Ok(exe) = std::env::current_exe() {
            if exe
                .file_name()
                .and_then(|n| n.to_str())
                .map(|n| n.eq_ignore_ascii_case("steklo"))
                .unwrap_or(false)
    ...
    fn resolve_setup_script() -> Option<PathBuf> {
        ...
        candidates.push(PathBuf::from(
            "/Applications/Steklo.app/Contents/Resources/setup_zsh.sh",
        ));
        candidates.push(
            config::HOME_DIR
                .join("Applications")
    ...
    fn maybe_setup_opencode_config() -> anyhow::Result<()> {
        ...
        if opencode_config.is_some() {
            if !prompt_yes_no("OpenCode config already exists. Overwrite with Steklo theme?") {
                return Ok(());
            }
        } else if !prompt_yes_no("Set up OpenCode with Steklo-matching theme?") {
            return Ok(());
        }
