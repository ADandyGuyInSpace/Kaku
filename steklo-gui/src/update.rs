...
fn get_github_release_info(uri: &str) -> anyhow::Result<Release> {
    ...
    let _res = Request::new(&uri)
        ...
        .header("User-Agent", &format!("steklo/{}", wezterm_version()))
        .send(&mut latest)
...
pub fn get_latest_release_info() -> anyhow::Result<Release> {
    get_github_release_info("https://api.github.com/repos/tw93/Steklo/releases/latest")
}
...
fn update_checker() {
    ...
    let force_ui = std::env::var_os("STEKLO_ALWAYS_SHOW_UPDATE_UI").is_some();
