...
fn resolve_bundled_steklo_bin() -> anyhow::Result<PathBuf> {
    ...
    #[cfg(target_os = "macos")]
    {
        add_candidate(
            &mut candidates,
            PathBuf::from("/Applications/Steklo.app/Contents/MacOS/steklo"),
        );
        add_candidate(
            &mut candidates,
            config::HOME_DIR
    ...
    anyhow::bail!(
        "could not find steklo binary; checked: {}",
        candidates
            .iter()
...
pub(crate) fn steklo_cli_program_for_spawn() -> String {
    match resolve_bundled_steklo_bin() {
        Ok(path) => path.to_string_lossy().into_owned(),
        Err(err) => {
            // Finder-launched apps can have a minimal PATH; fall back only when
            // we cannot resolve the bundled companion binary.
            log::warn!("Falling back to PATH lookup for `steklo`: {err:#}");
            "steklo".to_string()
        }
    }
}

pub fn open_steklo_config() {
    std::thread::spawn(move || {
        let result = (|| -> anyhow::Result<()> {
            let steklo_bin = resolve_bundled_steklo_bin()?;

            let ensure_status = Command::new(&steklo_bin)
                .arg("config")
                .arg("--ensure-only")
                .stdin(Stdio::null())
                .stdout(Stdio::null())
                .stderr(Stdio::null())
                .status()
                .with_context(|| format!("failed to launch {}", steklo_bin.display()))?;
            if !ensure_status.success() {
                anyhow::bail!(
                    "steklo config --ensure-only exited with status {}",
                    ensure_status.code().unwrap_or(-1)
                );
            }

            let home = std::env::var("HOME").context("resolve HOME for config path")?;
            let config_path = format!("{home}/.config/steklo/steklo.lua");
            ...
            let open_status = Command::new("/bin/sh")
                ...
                .stderr(Stdio::null())
                .status()
                .context("open steklo.lua in editor")?;
...
pub fn set_default_terminal_with_feedback() {
    ...
    match Connection::get() {
        Some(conn) => match conn.set_default_terminal() {
            Ok(()) => {
                let message = "Steklo is now the default terminal";
                if !show_window_toast(message) {
                    conn.alert("Default Terminal", message);
                }
            }
            Err(err) => {
                let message = format!("Failed to set Steklo as default terminal: {err:#}");
                log::error!("{message}");
            ...
            ApplicationEvent::PerformKeyAssignment(action) => {
                ...
                match action {
                    KeyAssignment::EmitEvent(event)
                        if event == "update-steklo" || event == "run-steklo-update" =>
                    {
                        let steklo_cli = steklo_cli_program_for_spawn();
                        spawn_command(
                            &SpawnCommand {
                    ...
                    KeyAssignment::EmitEvent(event) if event == "run-steklo-cli" => {
                        let steklo_cli = steklo_cli_program_for_spawn();
                        spawn_command(
                            &SpawnCommand {
                                args: Some(vec![steklo_cli]),
                                ..Default::default()
                            },
                            SpawnWhere::NewWindow,
                        );
                    }
                    KeyAssignment::EmitEvent(event) if event == "open-steklo-config" => {
                        open_steklo_config();
                    }
            ...
            while let Some(mux_window_id) = mux_windows.next() {
                ...
                if let Err(err) = TermWindow::new_window(mux_window_id).await {
                    ...
                    if err_text.contains("failed to create NSOpenGLPixelFormat") {
                        log::error!(
                            "OpenGL initialization failed. This often means no compatible GPU renderer is available (for example in some VMs). Try setting `front_end = 'WebGpu'` in steklo.lua or enabling VM GPU acceleration."
                        );
                    }
                    let mux = Mux::get();
