...
impl CommandDef {
    ...
    pub fn actions_for_palette_only(config: &ConfigHandle) -> Vec<ExpandedCommand> {
        ...
        let core_actions = [
            ...
            // Help menu
            ShowDebugOverlay,
            OpenUri("https://github.com/tw93/Steklo".to_string()),
            OpenUri("https://github.com/tw93/Steklo/issues/".to_string()),
        ];
        ...
        fn is_high_value_discovery_action(action: &KeyAssignment) -> bool {
            matches!(
                ...
                action,
                EmitEvent(name)
                    if name == "steklo-launch-lazygit"
                        || name == "steklo-launch-yazi"
                        || name == "run-steklo-ai-config"
            )
        }
    ...
    pub fn recreate_menubar(config: &ConfigHandle) {
        ...
        let steklo_perform_key_assignment_sel = sel!(stekloPerformKeyAssignment:);
        ...
        let main_menu = match Menu::get_main_menu() {
            Some(existing) => {
                mark_candidates(
                    ...
                    steklo_perform_key_assignment_sel,
                );
        ...
        let mut order: Vec<&'static str> =
            vec!["Steklo", "Shell", "Edit", "View", "Window", "Help"];
        ...
        fn command_rank_for_menu(title: &str, action: &KeyAssignment) -> usize {
            match title {
                ...
                "Shell" => match action {
                    ...
                    EmitEvent(name) if name == "run-steklo-ai-config" => 21,
                    EmitEvent(name) if name == "steklo-launch-lazygit" => 22,
                    EmitEvent(name) if name == "steklo-launch-yazi" => 23,
                ...
                "Help" => match action {
                    OpenUri(uri) if uri == "https://github.com/tw93/Steklo" => 10,
                    OpenUri(uri) if uri == "https://github.com/tw93/Steklo/discussions/" => 20,
                    OpenUri(uri) if uri == "https://github.com/tw93/Steklo/issues/" => 30,
                    ShowDebugOverlay => 90,
                    _ => 500,
                },
                _ => 1000,
        ...
        for &title in &order {
            ...
            for cmd in menu_commands {
                ...
                let mut submenu = main_menu.get_or_create_sub_menu(&cmd.menubar[0], |menu| {
                    if cmd.menubar[0] == "Window" {
                    ...
                    } else if cmd.menubar[0] == "Steklo" {
                        menu.assign_as_app_menu();

                        let about_item = MenuItem::new_with(
                            &format!("Steklo V{}", config::wezterm_version()),
                            Some(steklo_perform_key_assignment_sel),
                            "",
                        );
                        about_item.set_represented_item(RepresentedItem::KeyAssignment(
                            KeyAssignment::EmitEvent("run-steklo-cli".to_string()),
                        ));
                        ...
                        check_update.set_represented_item(RepresentedItem::KeyAssignment(
                            KeyAssignment::EmitEvent("run-steklo-update".to_string()),
                        ));
...
pub fn derive_command_from_key_assignment(action: &KeyAssignment) -> Option<CommandDef> {
    Some(match action {
        ...
        HideApplication => CommandDef {
            brief: "Hide Steklo".into(),
            doc: "Hide all Steklo windows".into(),
        ...
        EmitEvent(name) => {
            ...
            } else if name == "steklo-launch-lazygit" {
                CommandDef {
                    brief: "Lazygit".into(),
            ...
            } else if name == "steklo-launch-yazi" {
                CommandDef {
                    brief: "Yazi".into(),
        ...
        QuitApplication => CommandDef {
            brief: "Quit Steklo".into(),
            doc: "Quits Steklo".into(),
        ...
        OpenUri(uri) => match uri.as_ref() {
            "https://github.com/tw93/Steklo" => CommandDef {
                brief: "Star on GitHub".into(),
                doc: "Star Steklo on GitHub".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: None,
            },
            "https://github.com/tw93/Steklo/discussions/" => CommandDef {
                brief: "Discuss on GitHub".into(),
                doc: "Visit Steklo's GitHub discussion".into(),
                keys: vec![],
                args: &[],
                menubar: &[],
                icon: None,
            },
            "https://github.com/tw93/Steklo/issues/" => CommandDef {
...
fn compute_default_actions() -> Vec<KeyAssignment> {
    ...
    let mut actions = vec![
        ...
        SpawnTab(SpawnTabDomain::CurrentPaneDomain),
        SpawnWindow,
        EmitEvent("run-steklo-ai-config".to_string()),
        EmitEvent("steklo-launch-lazygit".to_string()),
        EmitEvent("steklo-launch-yazi".to_string()),
        SplitVertical(SpawnCommand {
            domain: SpawnTabDomain::CurrentPaneDomain,
            ..Default::default()
        }),
