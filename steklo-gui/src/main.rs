...
async fn async_run_terminal_gui(
    ...
    std::env::set_var("STEKLO_UNIX_SOCKET", unix_socket_path.clone());
                ...
                Ok(client) => {
                    ...
                    let res = block_on(executor.run(async move {
                        ...
                        if vers.config_file_path
                            != std::env::var_os("STEKLO_CONFIG_FILE").map(Into::into)
                        {
                            *self = Publish::NoConnectNoPublish;
                            anyhow::bail!(
                    ...
                    match res {
                        Ok(res) => {
                            log::debug!(
                             ...
                             Use steklo start --always-new-process if you do not want this behavior. \
                             Result={:?}",
                                res
                            );
                            Ok(true)
...
fn run_terminal_gui(opts: StartCommand, default_domain_name: Option<String>) -> anyhow::Result<()> {
    ...
    // First, let's see if we can ask an already running Steklo instance to do this.
...
fn fatal_toast_notification(title: &str, message: &str) {
    let should_show = if cfg!(debug_assertions) {
        std::env::var_os("STEKLO_DEV_FATAL_TOAST").is_some()
    } else {
        true
    };
...
fn run() -> anyhow::Result<()> {
    ...
    #[cfg(windows)]
    {
        unsafe {
            ::windows::Win32::UI::Shell::SetCurrentProcessExplicitAppUserModelID(
                ::windows::core::PCWSTR(wide_string("sh.steklo.Steklo").as_ptr()),
            )
            .unwrap();
        }
    }
    ...
    let sub = match opts.cmd.as_ref().cloned() {
        ...
        None => {
            ...
            let mut argv = vec!["steklo-gui".to_string()];
