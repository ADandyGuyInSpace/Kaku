...
impl TermWindow {
    ...
    fn created(&mut self, ctx: RenderContext) -> anyhow::Result<()> {
        ...
        match RenderState::new(ctx, &self.fonts, &self.render_metrics, ATLAS_SIZE) {
            Ok(render_state) => {
                log::debug!(
                    "OpenGL initialized! {} Steklo version: {}",
                    render_info,
...
impl TermWindow {
    ...
    pub fn perform_key_assignment(
        ...
        match assignment {
            ...
            EmitEvent(name) => {
                if name == "update-steklo" || name == "run-steklo-update" {
                    let steklo_cli = crate::frontend::steklo_cli_program_for_spawn();
                    self.spawn_command(
                        &SpawnCommand {
                            args: Some(vec![steklo_cli, "update".to_string()]),
                            domain: SpawnTabDomain::DomainName("local".to_string()),
                            ..Default::default()
                        },
                        SpawnWhere::NewWindow,
                    );
                } else if name == "run-steklo-cli" {
                    pane.writer().write_all(b"steklo\n")?;
                } else if name == "run-steklo-ai-config" {
                    pane.writer().write_all(b"steklo ai\n")?;
                } else if name == "steklo-toast-try-lazygit" {
                    self.show_toast("Try Lazygit: Cmd+Shift+G".to_string());
                } else if name == "steklo-toast-lazygit-no-pane" {
                    self.show_toast("Lazygit: No active pane".to_string());
                } else if name == "steklo-toast-lazygit-no-cwd" {
                    self.show_toast("Lazygit: Cannot detect current directory".to_string());
                } else if name == "steklo-toast-lazygit-not-git" {
                    self.show_toast("Lazygit: Not a git repository".to_string());
                } else if name == "steklo-toast-lazygit-missing" {
                    self.show_toast("Lazygit not found. Run steklo init".to_string());
                } else if name == "steklo-toast-lazygit-dispatch-failed" {
                    self.show_toast("Lazygit: Dispatch failed".to_string());
                } else if name == "steklo-toast-yazi-no-pane" {
                    self.show_toast("Yazi: No active pane".to_string());
                } else if name == "steklo-toast-yazi-missing" {
                    self.show_toast("Yazi not found. Run steklo init".to_string());
                } else if name == "steklo-toast-yazi-dispatch-failed" {
                    self.show_toast("Yazi: Dispatch failed".to_string());
                } else if name == "steklo-toast-ai-analyzing" {
                    let message = "Steklo Assistant analyzing command error".to_string();
                    self.show_ai_progress_toast(message.clone(), ai_toast_lifetime_ms(&message));
                } else if name == "steklo-toast-ai-ready" {
                    let message =
                        "Steklo Assistant suggestion ready. Press Cmd+Shift+E".to_string();
                    self.show_ai_result_notice(message.clone(), ai_toast_lifetime_ms(&message));
                } else if name == "steklo-toast-ai-unavailable" {
                    let message = "Steklo Assistant unavailable".to_string();
                    self.show_ai_result_notice(message.clone(), ai_toast_lifetime_ms(&message));
                } else if name == "steklo-toast-ai-missing-key" {
                    let message = "Run steklo ai to set up Steklo Assistant.".to_string();
                    self.show_ai_result_notice(message.clone(), ai_toast_lifetime_ms(&message));
                } else if name == "steklo-toast-ai-applied" {
                    // No notification on successful apply; command output is enough.
                } else if name == "steklo-toast-ai-no-pane" {
                    let message = "No active pane".to_string();
                    self.show_ai_result_notice(message.clone(), ai_toast_lifetime_ms(&message));
                } else if name == "steklo-toast-ai-no-suggestion" {
                    let message = "No executable suggestion".to_string();
                    self.show_ai_result_notice(message.clone(), ai_toast_lifetime_ms(&message));
                } else if name == "steklo-toast-ai-send-failed" {
                    let message = "Failed to apply suggestion".to_string();
                    self.show_ai_result_notice(message.clone(), ai_toast_lifetime_ms(&message));
                } else if name == "steklo-toast-ai-info" {
                    let message = "Steklo Assistant update".to_string();
                    self.show_ai_result_notice(message.clone(), ai_toast_lifetime_ms(&message));
                } else if let Some(payload) = name.strip_prefix("steklo-toast-ai-") {
                ...
                } else if name == "open-steklo-config" {
                    crate::frontend::open_steklo_config();
                } else if name == crate::frontend::SET_DEFAULT_TERMINAL_EVENT {
                    match Connection::get() {
                        Some(conn) => match conn.set_default_terminal() {
                            Ok(()) => {
                                self.show_toast("Steklo is now the default terminal".to_string());
                            }
                            Err(err) => {
                                log::error!("Failed to set Steklo as default terminal: {err:#}");
