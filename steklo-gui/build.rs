...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            VALUE "FileDescription",  "Steklo - A fast terminal for AI coding\\0"
            VALUE "FileVersion",      "{version}\\0"
            VALUE "LegalCopyright",   "Tw93, MIT licensed\\0"
            VALUE "InternalName",     "\\0"
            VALUE "OriginalFilename", "\\0"
            VALUE "ProductName",      "Steklo\\0"
...
"#,
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let build_target_dir = if let Ok(target_dir) = std::env::var("CARGO_TARGET_DIR") {
            ...
            // OUT_DIR is like: target/release-opt/build/steklo-gui-xxx/out
            // We need to get to: target/release-opt
            let out_dir = std::env::var("OUT_DIR").expect("OUT_DIR not set");
            let out_path = std::path::PathBuf::from(&out_dir);

            // Navigate up: out -> build -> steklo-gui-xxx -> release-opt
        ...
        println!("cargo:rerun-if-changed=assets/macos/Steklo.app/Contents/Info.plist");
